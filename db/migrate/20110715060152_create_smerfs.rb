class CreateSmerfs < ActiveRecord::Migration
  def self.up
    ...
    # Create link table between the usermodelnames and smerf_forms table 
    # which is used to record that the user completed and save the form
    create_table :smerf_forms_usermodelnames, :primary_key => :id, :force => true do |t|
      t.integer :usermodelname_id,       :null => false
    ...
    create_table :smerf_responses, :primary_key => :id, :force => true do |t|
      t.integer :smerf_forms_usermodelname_id, :null => false
  ...
  def self.down
    ...
    drop_table :smerf_forms_usermodelnames
